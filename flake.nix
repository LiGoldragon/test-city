  ...
  outputs = { self, nixpkgs, flake-utils, gascity-nix, gascity-v1-0, gascity-upstream-main, gascity-fork-issue-prefix, gascity-fork-dolt-amp, ... }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        runIdlePathGcExpanded = pkgs.writeShellApplication {
          name = "run-idle-path-gc-expanded";
          runtimeInputs = cityRuntimeDepsWithoutGascity ++ harnessDeps;
          text = ''
            export TEST_CITY_SOURCE_ROOT=${sourceRoot}
            exec ${pkgs.bash}/bin/bash ${sourceRoot}/scripts/run-idle-path-gc-expanded.sh "$@"
          '';
        };
        ...
        runIdlePathGcLifecycleChurn = pkgs.writeShellApplication {
          name = "run-idle-path-gc-lifecycle-churn";
          runtimeInputs = cityRuntimeDepsWithoutGascity ++ harnessDeps;
          text = ''
            export TEST_CITY_SOURCE_ROOT=${sourceRoot}
            exec ${pkgs.bash}/bin/bash ${sourceRoot}/scripts/run-idle-path-gc-lifecycle-churn.sh "$@"
          '';
        };
